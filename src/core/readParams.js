...
// `legacyMode` waives the allowlist on the query string only — 2.x had none.
// Never waived for a QUERY body: that route is new, so nothing to preserve.
...
// RFC 10008 §4: name the format this resource takes, so a 415 says what would
// have worked.
...
// An unknown key is a 400, not something to ignore: a quietly dropped
// `{"filters": …}` is a request for the whole collection.
export const parseQueryBody = (req, config) => {
    ...
    if (req.body === undefined) {
        // A server misconfiguration, and the log is the only place to say so.
        config.logger.error(
    ...
    // `limit` caps an unpaginated read, so pairing it with paging is a contradiction.
